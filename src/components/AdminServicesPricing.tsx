    ...
    try {
      ...
      toast({ 
        ...
        description: editedSaleConfig.is_active 
          ? `Promotion "${editedSaleConfig.sale_name}" activée (-${editedSaleConfig.discount_percentage}%)`
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-xl border border-border p-6">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {editedSaleConfig.is_active ? 'Promotion active sur tous les services' : 'Aucune promotion en cours'}
                </p>
              </div>
            </div>
            <Switch
          ...
          {/* Aperçu */}
          {editedSaleConfig.is_active && editedSaleConfig.discount_percentage > 0 && (
            <div className="p-4 bg-gradient-to-r from-neon-gold/10 to-primary/10 rounded-lg border border-neon-gold/30">
              <h4 className="font-semibold text-foreground mb-2">Aperçu des prix avec promotion</h4>
              <div className="space-y-2 text-sm">
                {services.map((service) => {
                  const originalPrice = editedPrices[service.id] || service.base_price;
                  const discountedPrice = originalPrice * (1 - editedSaleConfig.discount_percentage / 100);
                  return (
                    <div key={service.id} className="flex items-center gap-2">
                      ...
                      <span className="text-neon-gold font-bold">{discountedPrice.toFixed(2)}€</span>
                      <span className="text-green-500 text-xs">(-{editedSaleConfig.discount_percentage}%)</span>
                    </div>
                  );
                })}
              </div>
