...
async function run() {
  ...
  const delResult = await client.query("DELETE FROM services");
  console.log('Deleted all existing services:', delResult.rowCount);

  // Insert the 7 standard services
  ...
  // Verify
  const check = await client.query('SELECT service_key, name_fr, base_price, price_unit, is_active, sort_order FROM services ORDER BY sort_order');
  console.log('\nServices in DB:');
